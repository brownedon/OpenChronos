...
CC_INCLUDE = -I$(PROJ_DIR)/ -I$(PROJ_DIR)/include/ -I$(PROJ_DIR)/gcc/ -I$(PROJ_DIR)/driver/ -I$(PROJ_DIR)/logic/ -I$(PROJ_DIR)/bluerobin/ -I$(PROJ_DIR)/simpliciti/ -I$(PROJ_DIR)/simpliciti/Components/bsp -I$(PROJ_DIR)/simpliciti/Components/bsp/drivers -I$(PROJ_DIR)/simpliciti/Components/bsp/boards/CC430EM -I$(PROJ_DIR)/simpliciti/Components/mrfi -I$(PROJ_DIR)/simpliciti/Components/nwk -I$(PROJ_DIR)/simpliciti/Components/nwk_applications -I$(PROJ_DIR)/swap/ -I$(PROJ_DIR)/swap/bsp -I$(PROJ_DIR)/swap/mrfi/

CC_COPT		=  $(CC_CMACH) $(CC_DMACH) $(CC_DOPT)  $(CC_INCLUDE) 

LOGIC_SOURCE = logic/acceleration.c logic/alarm.c logic/altitude.c logic/battery.c  logic/clock.c logic/date.c logic/menu.c logic/stopwatch.c logic/temperature.c logic/test.c logic/user.c logic/phase_clock.c logic/eggtimer.c logic/prout.c logic/vario.c logic/sidereal.c logic/strength.c logic/sequence.c logic/gps.c
...
SWAP_SOURCE = swap/bsp/bsp.c swap/mrfi/mrfi.c swap/swapbrsr.c swap/swap.c swap/register.c swap/regtable.c swap/params.c swap/periodic.c

SWAP_O = $(addsuffix .o,$(basename $(SWAP_SOURCE)))

MAIN_SOURCE = ezchronos.c  gcc/intrinsics.c 

MAIN_O = ezchronos.o gcc/intrinsics.o 

ALL_O = $(LOGIC_O) $(SWAP_O) $(DRIVER_O) $(MAIN_O)

ALL_S = $(addsuffix .s,$(basename $(LOGIC_SOURCE))) $(addsuffix .s,$(basename $(DRIVER_SOURCE))) \
        $(addsuffix .s,$(basename $(SWAP_SOURCE))) $(addsuffix .s,$(basename $(MAIN_SOURCE)))  
